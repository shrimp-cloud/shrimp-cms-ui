...
<script setup name="ArticleCategoryEdit">
import { articleCategoryInfo, articleCategorySave, articleCategoryOptions } from "@/api/article";
...
function submitForm() {
  proxy.$refs["editRef"].validate(valid => {
    if (valid) {
      articleCategorySave(form.value).then(res => {
        if (res.code !== 1) {
