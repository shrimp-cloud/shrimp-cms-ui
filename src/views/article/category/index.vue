...
<script setup name="ArticleCategoryList">
import { articleCategoryList, articleCategoryRemove} from "@/api/article";
...
function getList() {
  loading.value = true;
  articleCategoryList(queryParams.value).then(res => {
...
function handleDelete(row) {
  proxy.$modal.confirm('是否确认删除名称为"' + row.categoryName + '"的数据项？').then(() => {
    articleCategoryRemove({id: row.id}).then(res => {
      if (res.code === 1) {
        getList();
