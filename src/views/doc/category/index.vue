...
<script setup name="DocCategoryList">
import { docCategoryList, docCategoryRemove} from "@/api/doc";
...
function getList() {
  loading.value = true;
  docCategoryList(queryParams.value).then(res => {
...
function handleDelete(row) {
  proxy.$modal.confirm('是否确认删除名称为"' + row.categoryName + '"的数据项？').then(() => {
    docCategoryRemove({id: row.id}).then(res => {
      if (res.code === 1) {
        getList();
