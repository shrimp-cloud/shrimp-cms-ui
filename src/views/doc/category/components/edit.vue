...
<script setup name="DocCategoryEdit">
import { docCategoryInfo, docCategorySave, docCategoryOptions } from "@/api/doc";
...
function handleEdit(row) {
  ...
  if (!row || !row.id) {
    ...
    docCategoryInfo({id:row.id}).then(res => {
      form.value = res.data;
      open.value = true;
      title.value = "修改";
      getCategoryOptions();
...
function submitForm() {
  proxy.$refs["editRef"].validate(valid => {
    if (valid) {
      docCategorySave(form.value).then(res => {
        if (res.code !== 1) {
